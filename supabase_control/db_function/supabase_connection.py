...
def connect_to_supabase(env_path=None, test_connection=True):
    ...
    # 2. 暫時重命名本地 supabase.py 以避免衝突（在 db_function 或上層 supabase_control 尋找）
    current_dir = os.path.dirname(os.path.abspath(__file__))
    parent_dir = os.path.dirname(current_dir)  # supabase_control
    supabase_py_path = os.path.join(current_dir, 'supabase.py')
    supabase_bak_path = os.path.join(current_dir, 'supabase.py.bak')
    if not os.path.exists(supabase_py_path):
        supabase_py_path = os.path.join(parent_dir, 'supabase.py')
        supabase_bak_path = os.path.join(parent_dir, 'supabase.py.bak')

    if os.path.exists(supabase_py_path) and not os.path.exists(supabase_bak_path):
        os.rename(supabase_py_path, supabase_bak_path)

    # 3. 載入環境變數（.env 放在 supabase_control 或 supabase_control/Erd）
    if env_path is None:
        possible_paths = [
            os.path.join(parent_dir, '.env'),           # supabase_control/.env
            os.path.join(parent_dir, 'Erd', '.env'),    # supabase_control/Erd/.env
            '.env'  # 當前工作目錄
        ]
        env_path = None

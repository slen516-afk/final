...
# 專案根目錄 = supabase_control（.env 所在處；腳本在 scripts/labeling/ 故需三層 parent）
...
# 4. 執行函數 (指定 ID)
# ==========================================
def process_specific_jobs(target_ids: List[int], table_name="job_posting"):
...
if __name__ == "__main__":
    ...
    if not test_ids:
        print("⚠️ 請在 test_ids 中填入要測試的 job_id，或改用 run_random_batch.py 隨機抽 500 筆。")
    else:

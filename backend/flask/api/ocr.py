...
def run_ocr_api():
    extract_func = current_app.config.get("OCR_HANDLER")
    if not extract_func:
    ...
    for index, file in enumerate(files):
        if index >= len(available_ids):
            ...
            processed_results.append(
                {"filename": file.filename, "status": "skipped", "message": msg}
            )
            continue
        ...
        try:
            ...
            event_response = (
                supabase.table("upload_event").insert(insert_event_payload).execute()
            )
            ...
            if isinstance(extracted_data, dict) and "error" in extracted_data:
                ...
                supabase.table("upload_event").update({"status": "failed"}).eq(
            ...
            supabase.table("upload_event").update({"status": "completed"}).eq(
                "event_id", inserted_event_id
            ).execute()
            ...
            resume_response = (
                supabase.table("resume").insert(insert_resume_payload).execute()
            )
            ...
            ocr_response = (
                supabase.table("ocr_result").insert(insert_ocr_payload).execute()
            )
            ...
            try:
                ...
                insert_profile_payload = {
                    ...
                    "privacy_settings": {
                        "privacy": extracted_data.get("structured_data", {}).get(
                            "privacy", "public"
                        )
                    },
                    "updated_at": now_str,

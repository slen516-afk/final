...
class TestStartAnalysisTask:
    ...
    def test_success(self, client, auth_headers):
        payload = {"resume_id": "r1", "survey_id": "s1"}
    ...
    def test_missing_fields(self, client, auth_headers):
        resp = client.post("/api/analysis/tasks", json={"resume_id": "r1"}, headers=auth_headers)
        assert resp.status_code == 400
        assert "survey_id" in resp.get_json()["error"]

    def test_no_auth(self, client):
        resp = client.post("/api/analysis/tasks", json={"resume_id": "r1", "survey_id": "s1"})
...
# ─── D-03  GET /api/analysis/tasks/<id>/results ──────────────────────────────

class TestGetOptimizationResults:

    def test_done(self, client, auth_headers, seed_job):
        result = json.dumps({"career_readiness_score": 90})
        jid = seed_job("job_r1", status="done", result=result)
